...
const Sidebar = () => {
  ...
  const [triggerGetUserById, { data: fetchResult }] = useLazyGetUserByIdQuery();
  ...
  useEffect(() => {
    if (fetchResult?.success && fetchResult?.data) {
      dispatch(setUserData(fetchResult.data));
    }
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <div
          className={`flex items-center h-16 border-b border-gray-800 px-4 ${sidebarCollapsed ? "justify-center" : "gap-3"}`}
        >
          ...
          {menuItems.map(({ path, label, icon: Icon }) => {
            ...
            return (
              <NavLink
                ...
                to={path}
                onClick={() =>
                  window.innerWidth < 1024 && dispatch(setSidebarOpen(false))
                }
              ...
              >
                ...
                <Icon
                  size={19}
                  className={`flex-shrink-0 ${isActive ? "text-violet-400" : "text-gray-500 group-hover:text-gray-300"}`}
                />
                {!sidebarCollapsed && <span>{label}</span>}
                {sidebarCollapsed && (
                  <div
                    className="absolute left-full ml-3 px-2.5 py-1.5 bg-gray-800 border border-gray-700 rounded-md text-xs text-gray-200 
